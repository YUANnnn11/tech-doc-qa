...
def create_index_from_files(file_names: list[str], force_rebuild: bool = False):
    """
    从指定的文件列表创建或更新 FAISS 索引。
    file_names: 在 UPLOAD_FOLDER 中的文件名列表。
    force_rebuild: True 时清空旧索引与已处理记录，从零全量重建（用于删除/刷新场景，避免已删文件的向量残留）。
    """
    logger.info(f"开始从文件列表创建/更新 FAISS 索引: {file_names} (force_rebuild={force_rebuild})")
    doc_paths = [os.path.join(UPLOAD_FOLDER, fn) for fn in file_names]
    all_docs = []

    if force_rebuild:
        # 全量重建：清空已处理记录，并删除旧索引，确保已删文件的向量被清除
        processed_files = set()
        if os.path.exists(FAISS_INDEX_PATH):
            shutil.rmtree(FAISS_INDEX_PATH)
        os.makedirs(FAISS_INDEX_PATH, exist_ok=True)
        logger.info("force_rebuild=True：已清空旧索引与已处理记录，将从零重建。")
    else:
    ...
    for doc_path in doc_paths:
        ...
        loader = None
        try:
            if doc_path.lower().endswith((".txt", ".md", ".markdown")):
                logger.info(f"正在加载文本/Markdown 文件: {file_name_for_log}...")
    ...
    if not all_docs:
        if force_rebuild:
            # 没有文件需要重建（例如删光了所有文档）：清空索引即为成功
            save_processed_files(set())
            logger.info("force_rebuild=True 且无文件：索引已清空。")
            return True
    ...
    try:
        ...
        # 分批 embedding：一次性处理太多 chunk 会触发 Ollama 的 tokenize 子进程崩溃（Windows 下约 300 条触发）
        BATCH_SIZE = 100
        ...
        for i in range(0, len(split_docs), BATCH_SIZE):
            ...
            logger.info(f"正在嵌入第 {i // BATCH_SIZE + 1} 批（{len(batch)} 个片段）...")
...
def rebuild_index_from_all_files():
    ...
    try:
        files = [f for f in os.listdir(UPLOAD_FOLDER) if f.lower().endswith(('.pdf', '.txt', '.md', '.markdown'))]
        if not files:
